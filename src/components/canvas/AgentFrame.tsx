...
export default function AgentFrame({ data, id }: any) {
    ...
    const danceSearchSections = useMemo(() => {
        // Drafts attached to this performer
        const attachedDraftItems: DanceSearchItem[] = (performer?.danceRefs || [])
        ...
        // All dance drafts in the store (not yet attached to this performer)
        const attachedDraftIds = new Set(
            (performer?.danceRefs || [])
                .filter((ref) => ref.kind === 'draft')
                .map((ref) => ref.draftId),
        );
        const standaloneDraftItems: DanceSearchItem[] = Object.entries(drafts)
        ...
        const draftItems = [...attachedDraftItems, ...standaloneDraftItems];

        const performerItems: DanceSearchItem[] = (performer?.danceRefs || [])
            .filter((ref) => ref.kind !== 'draft')
            .map((ref) => ({
        ...
        const filteredAssetItems: DanceSearchItem[] = danceAssets
        ...
        const sections: Array<{ key: string; title: string; items: DanceSearchItem[] }> = [
            {
                key: 'draft',
                title: 'Draft',
            ...
            },
            {
                key: 'stage',
                title: 'Stage',
                items: filteredAssetItems.filter((item) => item.scope === 'stage').filter(byQuery),
            },
            {
                key: 'global',
                title: 'Global',
                items: filteredAssetItems.filter((item) => item.scope === 'global').filter(byQuery),
            },
        ]
    ...
    const handleSend = useCallback(() => {
        ...
        const formattedAttachments = attachments.map(a => ({
            ...
            url: a.absolute.startsWith('data:') ? a.absolute : `file://${a.absolute}`,
            filename: a.name
        }));
