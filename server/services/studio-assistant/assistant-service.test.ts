...
describe('ensureAssistantAgent', () => {
    ...
    it('tells the assistant to avoid invented ids and string actRules payloads', async () => {
        ...
        expect(prompt).toContain('load `studio-assistant-tal-design-guide` and prefer one short confirmation question that proposes a role-appropriate Tal draft')
        expect(prompt).toContain('model variants')
        expect(prompt).toContain('Do not invent variant ids')
        expect(prompt).toContain('Do not default to creating new Performers without Tal')
